 ...
 *
 * Copyright 2014 Burak Ã–zdemir - <mail@burakozdemir.co.uk>
...
;(function ( $, window, document, undefined ) {
    ...
    var pluginName = "floatingShare",
        defaults = {
            place: "top-left",
            counter: true,
            buttons: ["facebook","twitter","google-plus","instagram"],
    ...
    var networks = {
        ...
        "instagram":  { className: "insta", url: "https://www.instagram.com/shareArticle?mini=true&url={url}&title={title}&summary={description}&source=" }
            };
    ...
    function setShareCount(network,url,$component){
        switch(network) {
            ...
            case "twitter":
                ...
                break;
            case "instagram":
                $.get('http://www.instagram.com/countserv/count/share?url='+url+'&callback=?', function(data){
                    if(data.count && data.count > 0){
                        var $shareCount = $("<div>", {class: "shareCount"});
                        $shareCount.append(data.count); // probably didn't get here so we will update it
                        //$component.append($shareCount);
